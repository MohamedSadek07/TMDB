...
extension MovieDetailsResponseModel {
    func toDetails() -> MovieDetails {
        MovieDetails(
            id: id ?? 0,
            title: title ?? "",
            backdropURL: Constants.postureBaseUrl + (backdropPath ?? ""),
            posterURL: Constants.postureBaseUrl + (posterPath ?? ""),
            ...
            budget: budget ?? 0,
            revenue: revenue ?? 0,
            spokenLanguages: spokenLanguages?.compactMap { $0.name } ?? [],
            status: status ?? "",
            runtime: runtime ?? 0
        )
    }
